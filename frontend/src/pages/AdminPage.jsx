...
function CommunityModal({ community, onClose, onSave }) {
  const [form, setForm] = useState({
    ...
    nameSwahili: community && community.nameSwahili || '',
    description: community && community.description || '',
    descriptionSwahili: community && community.descriptionSwahili || '',
    region: community && community.region || '',
    country: community && community.country || 'Kenya',
  });
  ...
  const handleSave = async () => {
    ...
    catch (err) { toast.error(err.response && err.response.data && err.response.data.error || 'Failed to save'); }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
      <div className="card" style={{ padding: '1.5rem', width: '100%', maxWidth: '32rem', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.25rem', color: '#8B6F35', lineHeight: 1 }}>x</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            <div><label style={labelStyle}>Name (English) *</label><input className="input" value={form.name} onChange={set('name')} placeholder="e.g. Rendille Community" /></div>
            <div><label style={labelStyle}>Name (Swahili)</label><input className="input" value={form.nameSwahili} onChange={set('nameSwahili')} placeholder="e.g. Jamii ya Warendille" /></div>
          </div>
          <div><label style={labelStyle}>Description (English)</label><textarea className="input" rows={3} value={form.description} onChange={set('description')} placeholder="Describe the community..." /></div>
          <div><label style={labelStyle}>Description (Swahili)</label><textarea className="input" rows={3} value={form.descriptionSwahili} onChange={set('descriptionSwahili')} placeholder="Maelezo kwa Kiswahili..." /></div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            <div><label style={labelStyle}>Region</label><input className="input" value={form.region} onChange={set('region')} placeholder="e.g. Marsabit County" /></div>
            <div><label style={labelStyle}>Country</label><input className="input" value={form.country} onChange={set('country')} /></div>
          </div>
        </div>
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid #EDE4D3' }}>
          ...
          <button onClick={handleSave} disabled={saving} className="btn-primary">{saving ? 'Saving...' : community ? 'Save Changes' : 'Add Community'}</button>
        </div>
      </div>
    </div>
  );
...
export default function AdminPage() {
  ...
  const handleReview = async (id, decision) => {
    try { await knowledgeApi.review(id, { decision }); qc.invalidateQueries(['knowledge', 'pending']); toast.success('Review submitted'); }
    catch { toast.error('Review failed'); }
  };

  const handleSaveCommunity = async (data) => {
    if (communityModal === 'new') { await communityApi.create(data); toast.success('Community added!'); }
    else { await api.patch('/communities/' + communityModal.id, data); toast.success('Community updated!'); }
    qc.invalidateQueries(['communities']);
  };

  const handleRemove = async (c) => {
    if (!window.confirm('Remove ' + c.name + '?')) return;
    try { await api.delete('/communities/' + c.id); qc.invalidateQueries(['communities']); toast.success('Removed'); }
  ...
  const tabStyle = (id) => ({
    ...
    borderRadius: '0.5rem 0.5rem 0 0', border: '1px solid transparent', background: 'none',
    ...
    <div>
      ...
      <div style={{ maxWidth: '80rem', margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.25rem', borderBottom: '1px solid #D9C9A8', marginBottom: '2rem' }}>
          {TABS.map(t_ => <button key={t_.id} onClick={() => setTab(t_.id)} style={tabStyle(t_.id)}>{t_.label}</button>)}
        </div>

        {/* Review Queue */}
        {tab === 'review' && (
          pendingLoading ? <Spinner /> : !pending || !pending.length ? (
            <div style={{ textAlign: 'center', padding: '4rem', color: '#A88B50' }}>
              <p style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>All clear</p>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1, minWidth: '200px' }}>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.5rem' }}>
                        <TierBadge tier={r.accessTier} /><CategoryBadge category={r.category} />
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '1rem', marginTop: '0.75rem', fontSize: '0.75rem', color: '#A88B50', flexWrap: 'wrap' }}>
                        <span>By: {r.contributor && r.contributor.name}</span>
                        <span>Community: {r.community && r.community.name}</span>
          ...
          usersLoading ? <Spinner /> : (
            <div className="card" style={{ overflow: 'hidden' }}>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', fontSize: '0.875rem', borderCollapse: 'collapse', minWidth: '600px' }}>
                  <thead style={{ background: '#F7F3ED', borderBottom: '1px solid #EDE4D3' }}>
                    <tr>
                      {['Name', 'Email', 'Role', 'Access', 'Verified', 'Joined'].map(h => (
                        <th key={h} style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.75rem', fontWeight: 600, color: '#8B6F35', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {users && users.map((u, i) => (
                      <tr key={u.id} style={{ borderBottom: '1px solid #EDE4D3', background: i % 2 === 0 ? 'white' : '#FDFAF6' }}>
                        ...
                        <td style={{ padding: '0.75rem 1rem' }}><span style={{ width: '0.625rem', height: '0.625rem', borderRadius: '50%', display: 'inline-block', background: u.isVerified ? '#538C1A' : '#C4AD7C' }} /></td>
                        <td style={{ padding: '0.75rem 1rem', color: '#A88B50', whiteSpace: 'nowrap' }}>{format(new Date(u.createdAt), 'MMM d, yyyy')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {tab === 'communities' && (
          <div>
            ...
            {commLoading ? <Spinner /> : !communities || !communities.length ? (
              <div style={{ textAlign: 'center', padding: '4rem', color: '#A88B50' }}>
                <p style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>No communities yet</p>
                  ...
                  <div key={c.id} className="card" style={{ padding: '1.25rem' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
                      <div style={{ flex: 1, minWidth: '200px' }}>
                        ...
                        {c.description && <p style={{ fontSize: '0.875rem', color: '#6E5528', lineHeight: 1.5 }}>{c.description}</p>}
                        <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem', fontSize: '0.75rem', color: '#A88B50', flexWrap: 'wrap' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0 }}>
                        ...
                        <button onClick={() => handleRemove(c)} className="btn-ghost" style={{ fontSize: '0.875rem', color: '#C44420' }}>Remove</button>
                      </div>
                    </div>
                  </div>
                ))}
