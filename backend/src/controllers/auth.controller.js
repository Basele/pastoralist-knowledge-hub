...
exports.register = async (req, res, next) => {
  try {
    ...
    const user = await prisma.user.create({
      data: { email, passwordHash, name, nameSwahili, communityId, role: "COMMUNITY_MEMBER", accessTier: "PUBLIC" },
      select: { id: true, email: true, name: true, role: true, accessTier: true, createdAt: true },
    });
    const { accessToken, refreshToken } = await generateTokens(user);
    await prisma.refreshToken.create({
      data: { id: uuidv4(), token: refreshToken, userId: user.id, expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) },
    });
...
exports.refresh = async (req, res, next) => {
  try {
    ...
    await prisma.refreshToken.create({
      data: { id: uuidv4(), token: tokens.refreshToken, userId: user.id, expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) },
    });
    res.json(tokens);
  } catch (err) { next(err); }
};
